...
## Recent Changes
- 2026-03-30: Build fix — removed react-icons/si dependency (replaced 6 social icons with inline SVGs in social-icons.tsx), eliminated OOM build failure on low-RAM servers; deploy.sh now uses NODE_OPTIONS=--max-old-space-size=1024; script/build.ts now backs up old dist and restores it if new build fails (zero-downtime safety)
